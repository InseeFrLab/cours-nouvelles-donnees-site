...
Les réseaux sociaux constituent une source privée de données numériques pouvant, dans certains cas, être associées à une information de localisation. Bien que l’accès à ces données soit aujourd’hui plus restreint qu’auparavant, elles continuent de présenter un potentiel d’intérêt pour la statistique publique, sous réserve de fortes contraintes méthodologiques, juridiques et techniques.
  ...
  - En complément d’autres sources massives, telles que les données de téléphonie mobile, les données issues des réseaux sociaux peuvent contribuer à l’analyse des dynamiques spatiales et temporelles de certaines populations. Elles peuvent notamment apporter un éclairage spécifique sur les flux touristiques, les touristes étant souvent sous-représentés dans les données de téléphonie nationale, mais relativement actifs sur certaines plateformes sociales. Il convient toutefois de souligner que seule une faible proportion des publications est effectivement géolocalisée, cette fonctionnalité reposant sur un choix explicite de l’utilisateur.

  - Les contenus publiés sur les réseaux sociaux peuvent également être mobilisés pour estimer des indicateurs agrégés de bien-être ou d’humeur collective, à partir de méthodes d’analyse de sentiment appliquées aux textes. De même, ces données peuvent servir à analyser l’opinion publique à propos de sujets ou d’événements spécifiques. À titre d’exemple, Statistics Netherlands (CBS) a expérimenté l’utilisation d’indicateurs de sentiment dérivés de données issues des réseaux sociaux afin de compléter des indicateurs de confiance des consommateurs traditionnellement calculés à partir d’enquêtes [@brakel-17].

Du point de vue de l’accès aux données, plusieurs plateformes proposent (ou proposaient) des interfaces de programmation (API) permettant la collecte automatisée de contenus publics. Toutefois, les conditions d’accès ont évolué de manière significative ces dernières années. L’exemple de X/Twitter est emblématique : alors que son API permettait historiquement de récupérer des tweets publics à partir de requêtes simples (mots-clés, filtres temporels, exclusion des retweets, présence d’une géolocalisation), l’accès gratuit à ces fonctionnalités a été fortement restreint, voire supprimé pour de nombreux usages.

Lorsque la donnée est disponible, la géolocalisation associée à un message peut prendre différentes formes : coordonnées GPS précises, zone géographique (ville, lieu) ou simple référence à un emplacement nommé. Dans tous les cas, cette information n’est présente que si l’utilisateur a explicitement activé ou renseigné une localisation au moment de la publication. Il est alors possible, pour les messages géolocalisés, de connaître l’heure et le lieu approximatif ou exact de l’émission du contenu, sous réserve des limites imposées par la plateforme.

:::{.callout-note}
Des forts biais de sélection peuvent exister lorsque l'on exploite les données de réseaux sociaux.
Dans le cas de `X/Twitter` par exemple, les personnes qui postent du contenu, et *a fortiori* les personnes qui partagent leur localisation, ne constituent _a priori_ pas un échantillon représentatif de la population générale.
