...
[^IA] : Vous avez le droit d'utiliser votre assistant IA préféré ! Mais ne prenez pas pour argent comptant ce qu'il vous propose.
...
- 2.1 : Transformer l'objet sf `triangle` en WKT après l'avoir reprojeté en Lambert 2154 (même système de projection que les transactions DVF, voir @tip-geom).
- 2.2 : Faire un filtre attributaire sur la commune de Malakoff.
- 2.3 : Créer une colonne "in_triangle" booléenne et égale à TRUE pour les transactions présentes dans le triangle (utiliser la colonne "geometry", au format WKB, voir @tip-wkb).
- 2.4 : Calculer les médianes en groupant sur la variable "in_triangle".
:::
...
Pour l'exercice 3.2, nous aurons besoin de la structure de requête suivante pour créer la variable "in_triangle" :
...
```{r}
medians_in_out <- transactions_malakoff %>% group_by(in_triangle) %>% summarise(median = median(valeur_fonciere))
