...
Les données ont été préparées en amont de ce TD. Un lien de lancement rapide est disponible ci-dessus qui met à disposition un environnement pré à l'emploi sur le [SSPCloud](https://datalab.sspcloud.fr/). 
...
On peut se rassurer, on obtient la même chose avec `duckdb`:
...
La médiane des prix est un peu plus élevée dans le Triangle qu'en dehors. On peut aller au-delà et étudier la distribution des transactions. Bien que la taille d'échantillon soit réduite, on a ainsi une idée de la diversité des prix à Malakoff.
