...
Vous pourrez réaliser ce TP dans un service préconfiguré du `SSP Cloud` en clicand sur le lien ci-dessous : 
...
L'idée de ce tutoriel est de classer des déclarations d'entreprises dans une nomenclature générique des activités productives. Celle-ci permet de produire de nombreuses statistiques économiques sectorielles sur le tissu productif français. L'Insee ayant vocation à produire des statistiques agrégées sur de nombreuses questions, cette approche constitue l'un des principaux cas d'application du NLP pour l'Institut dans des domaines aussi divers que la classification dans une nomenclature d'activités (NAF), une nomenclature de professions (PCS), de produits (COICOP), de lieux géographiques, etc.
...
Le premier exercice a vocation à illustrer la manière classique de rentrerdans un corpus de données textuelles. 
...
Dans une démarche exploratoire, le plus simple est de commencer par compter les mots de manière indépendante (approche sac de mot). 
...
Néanmoins, à ce stade, les données sont encore très bruitées. 
...
```{python}
wordcloud_corpus_cleaned = graph_wordcloud(train, "text_clean")
...
1. Encoder les labels et découper les données en _train_, __val__ et _test_.
...
`ttc` est un framework de codification automatique, basé sur `torch` et qui permet de contruire des modèles de codification automatique.
...
A noter que `ttc` est une librarie opensource, développée et maintenue par l'Insee (unité SSPLab).

La première étape consiste à instancier un tokenizer.
Nous allons utiliser ici le tokenizer basique de `Fasttext` (découpage en mots, ngrams de mots et ngrams de caractères).

2. Instancier un tokenizer `NGramTokenizer` (essayer à partir de la documentation de `torchTextClassifiers` - ou en reprenant le code ci-dessous)
...
3. Etudiez le code dans l'aide ci-dessous et lancez-le pour configurer et entraînez votre modèle
...
4. Calculer l'accuracy du modèle. Etonnant ?

5. Tester le modèle sur quelques libellés de professions : 

```{.python}
searched_professions = np.array(["data scientist", "Commercial dans l'automobile", "peintre"])
```
...
# Download the model
# In terminal : 
os.mkdirs("model_ape/", exist_ok=True)
os.system("curl -O https://minio.lab.sspcloud.fr/projet-formation/nouvelles-sources/model_ape/metadata.pkl --output applications/model_ape/metadata.pkl")
os.system("curl -O https://minio.lab.sspcloud.fr/projet-formation/nouvelles-sources/model_ape/model_checkpoint.ckpt --output model_ape/model_checkpoint.ckpt")
os.system("curl -O https://minio.lab.sspcloud.fr/projet-formation/nouvelles-sources/model_ape/tokenizer.pkl --output model_ape/tokenizer.pkl")
