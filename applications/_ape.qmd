---
title: "Classification automatique dans une nomenclature"
subtitle: "Application de `Fasttext`"
echo: false
execute: 
  cache: true
---
...
la classification automatiquem par le biais de
...
groupe donné
dans le corpus. La tester avec _"data science"_ et _"boulanger"_
...
def nb_occurrence(train_data, word):
    word_capitalized = word.upper() 
    mask = train_data['text'].str.contains(word_capitalized)
    nb_occurrence = mask.astype(int).sum()
    print(
        f"Nombre d'occurrences du terme '{word}': {nb_occurrence}"
    )
...
Les _wordcloud_ peuvent servir à rapidement visualiser
...
les abbréviations, etc. qui peuvent bruiter notre
corpus. 
...
et la performance classe par classe (pour les classes de plus
de 200 cas). 
...
aggregation.loc[aggregation['total']>200].sort_values('accuracy', ascending = True)
```
